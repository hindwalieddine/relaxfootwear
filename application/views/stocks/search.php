...
<div class="search-summary">
    <div id="toolbox" class="noprint">
        ...
        <div class="noprint">
            <?php echo form_open('/stocks/search', array('method' => 'get')) ?>
            <table>
                <tr>
                    <td>  <label>Article:</label></td>
                    <td><?php echo form_input('article_id', set_value('article_id', $article_id), 'data-url="/relax/ajax/articles"') ?></td>
    ...
    <div style="width:950px; overflow: auto; height: 350px">
        <table cellspacing="0">
            ...
            ?>
            <?php
            foreach (resultdepot as $row1) :
                $total1 = $row1->qty1 + $row1->qty2 + $row->qty3 + $row->qty4 + $row->qty5 + $row->qty6 + $row->qty7 + $row->qty8 + $row->qty9 + $row->qty10 + $row->qty11 + $row->qty12 + $row->qty13;
                $purchaseprice = $purchaseprice + ($row1->purchase_price*$total1);



                if (
                        strpos($row1->article_code, '1') === 0 ||
                        strpos($row1->article_code, '2') === 0 ||
                        strpos($row1->article_code, '3') === 0 ||
                        strpos($row1->article_code, '4') === 0 ||
                        strpos($row1->article_code, '5') === 0 ||
                        strpos($row1->article_code, '7') === 0
                ) {
                    $counts['shoes'] += $total1;
                    $total1s['shoes'] += $total1 * $row1->sale_price;
                } else if (strpos($row1->article_code, '6') === 0) {
                    $counts['accessories'] += $total1;
                    $total1s['accessories'] += $total1 * $row1->sale_price;
                } else if (strpos($row1->article_code, '8') === 0) {
                    $counts['bags'] += $total1;
                    $total1s['bags'] += $total1 * $row1->sale_price;
                } else if (strpos($row1->article_code, '9') === 0) {
                    $counts['bas'] += $total1;
                    $total1s['bas'] += $total1 * $row1->sale_price;
                }
                ?>
            <?php if ($total1 != 0){ ?>
                <tr align="center" class="row<?php echo $r ?>">
                    <td width="50"><a href="<?php echo config_item('base_url') ?>articles/view/<?php echo $row1->article_id ?>"><?php echo $row1->article_code ?></a></td>
                    <td width="50"><?php echo $row1->sale_price ?></td>
                    <td width="50"><?php echo $row1->qty1 ?></td>
                    <td width="40"><?php echo $row1->qty2 ?></td>
                    <td width="40"><?php echo $row1->qty3 ?></td>
                    <td width="40"><?php echo $row1->qty4 ?></td>
                    <td width="40"><?php echo $row1->qty5 ?></td>
                    <td width="40"><?php echo $row1->qty6 ?></td>
                    <td width="40"><?php echo $row1->qty7 ?></td>
                    <td width="40"><?php echo $row1->qty8 ?></td>
                    <td width="40"><?php echo $row1->qty9 ?></td>
                    <td width="40"><?php echo $row1->qty10 ?></td>
                    <td width="40"><?php echo $row1->qty11 ?></td>
                    <td width="40"><?php echo $row1->qty12 ?></td>
                    <td width="40"><?php echo $row1->qty13 ?></td>
                    <td width="40"><b><?php echo $total1 ?></b></td>
                    <td><?php echo $total1 * $row1->sale_price ?> </td>
